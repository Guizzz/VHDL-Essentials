...
export function lintQsfText(text: string): vscode.Diagnostic[]
{
    ...
    checkDuplicateQsfAssignments(lines, diagnostics);

    return diagnostics;
}

function checkDuplicateQsfAssignments(lines: string[], diagnostics: vscode.Diagnostic[]): void
{
    ...
    for (const [, group] of byPin)
    {
        ...
        {
            const range = new vscode.Range(a.line, 0, a.line, lines[a.line].length);
            diagnostics.push(
                new vscode.Diagnostic(
                    range,
